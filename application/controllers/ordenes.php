...
class Ordenes extends BaseController
{
    ...
    function reparacionesOrdenes()
    {
        ...
        //die('<pre>'.print_r($socios,TRUE).'<pre>');
          ...
          $viewdata['ordenes'][$i]->equipo_enviado_al_socio_no_resuelto = $this->socios_model->equipo_enviado_al_socio_no_resuelto($viewdata['ordenes'][$i]->rm_id);

          $viewdata['ordenes'][$i]->categorias_vigentes = $this->ordenes_model->get_categorias_vigentes_ot($viewdata['ordenes'][$i]->rm_id);
    ...
    function instalacionesOrdenes()
    {
        ...
        $viewdata['ordenes']    = $this->ordenes_model->get_ordenes_asignadas_a_instalaciones();
    ...
    function reparacionesRechazadas()
    {
        ...
        $select =  "rm.id as rm_id,
                    rm.serie as rm_serie, 
                    mu.descrip as mu_descrip,
                    re.fecha as re_fecha, 
                    fm.descrip as fm_descrip, 
                    dm.descrip as dm_descrip";
        ...
        $this->db->where('rm.ultimo_estado', 8);
        $this->db->where('re.tipo', 8);
        $this->db->where("(rm.tipo = 'R' OR rm.ultimo_categoria = 1)");
        ...
        if ($returns["page"] != NULL) {
          $this->db->limit($returns["page"], $returns["segment"]);
        }
        $query = $this->db->get();
        // pre($this->db->last_query());die;
        $viewdata['ordenes'] = $query->result();
    ...
    function reparacionesFinalizadas()
    {
        ...
        $this->db->from('reparacion_main as rm');
        $this->db->join('reparacion_estados as re', 'rm.id = re.orden','inner');
        $this->db->join('fallas_main as fm', 'fm.id = rm.falla','inner');
        $this->db->join('municipios as mu', 'mu.id = rm.idproyecto','inner');
        $this->db->join('diagnosticos_main as dm', 'dm.id = rm.diagnostico','inner');

        if (in_array($role,array(101,102,103,104,105))) {
          $this->db->join('municipios_asignaciones as MA', 'MA.id_proyecto = mu.id','inner');
        }
    
        if(!empty($searchText)) {
          $busqueda = "(
                  fm.descrip LIKE '%".$searchText."%'
                  OR rm.serie LIKE '%".$searchText."%'
                  OR mu.descrip LIKE '%".$searchText."%'
                )";
          $this->db->where($busqueda);
        }

        $this->db->where('rm.ultimo_estado', 7);
        $this->db->where('re.tipo', 7);
        $this->db->where("(rm.tipo = 'R' OR rm.ultimo_categoria = 1)");
    ...
    function reportesDesestimados()
    {
        ...
        $count   = $this->ordenes_model->getCountOrdenes($estados,NULL,NULL,$viewdata['searchText'],$this->session->userdata('userId'), $this->role);
    ...
    function altanuevaorden()
    {
        ...
        if(!$this->ordenes_model->altaNuevaOrden($data_orden,$data_estado)){
          $this->session->set_flashdata('error',"Error al aprobar la solicitud  (error de transacción)");
        }else{
    ...
    function altanuevasolicitud()
    {
        ...
        if (!$this->ordenes_model->altaNuevaSolicitud($data_orden,$data_estado)) {
            die("Error al aprobar la solicitud (error de transacción)");
        }
    ...
    { 
        ...
        $ot_nuevas_categorias_vigentes = array_filter($this->input->post('nuevas_categorias_vigentes'));
        $ot_nuevas_categorias_vigentes = array_values($ot_nuevas_categorias_vigentes);
        // pre($ot_nuevas_categorias_vigentes, "pe");
        ...
        $data_estado = array(
            ...
            'nuevas_categorias_vigentes' => $ot_nuevas_categorias_vigentes
        );

        $list = $this->municipios_model->traerListaMunicipio();

        $found = false;

        foreach ($list as $item) {
          if ($item->id == $orden->mu_id) {
              $found = true;
              break;
          }
        }
        ...
        $data_mail = array(
          ...
          'fecha' => $fecha,
          'observ' => $observ,
          'Estado_nue' => $this->input->post('estado'),
          'Categoria_nue' =>$this->input->post('categoria'),          
          'proyecto'	=> $found,
          'proyecto_desc'	=> $orden->mu_descrip,
          'socio'	=> $orden->emo_asociado,
          'Estado_ant'	=> $orden->em_estado,
          'Categoria_ant'	=> $orden->rc_id,
          'Equipo'	=> $orden->em_serie_int,
        );



        // pre($data_mail);die; //proyecto
        // pre($orden->emo_asociado);die; //Socio MS-TRAFFIC
        // pre($orden->ema_descrip);die; //Socio MSTRAFFIC

        // pre($orden->rc_descrip);die; //estado_ant bien

        
        // pre($orden->rc_id);die; //estado_ant bien

        // pre($orden->em_estado);die; //estado_ant bien

        
        
        $respuesta = sector_mail($data_mail);
        ...
        // pre($datos_mail,$mails_to,$mails_cc,$mails_cco,$orden->rm_serie,$orden->mu_descrip,$observ,$fecha,$id_orden);die;
