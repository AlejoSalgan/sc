...
class Historial_model extends CI_Model
{
    ...
    {
        $this->db->select('FM.descrip as falla, RE.fecha, RM.nro_msj');
        // , OM.bajada_observ as observacion, OM.tecnico, U.name as nameTecnico
        ...
        $result_query_local = $this->db->get()->result();

        // bloque apra añadir data de API factory a consulta local
        $options = array(
            "exclude_empty_fields" => true,
            "exclude_duplicate_fields" => true,
            "sort_list_by_field" => true
        );
        $nro_mensajes = get_list_field_in_array($result_query_local, "nro_msj", $options);
        $data = array(
            "fields" => array("nro_msj", "ordenesb_main.bajada_observ as observacion", "ordenesb_main.tecnico", "U.name as nameTecnico"),
            "joins" => array(
                "tbl_users as U,U.userId=ordenesb_main.tecnico,left",
            ),
            "wherein" => array("ordenesb_main.nro_msj", join(",", $nro_mensajes))
        );
        $responseApi = json_decode(callAPI("GET", API_FACTORY["ordenb_get"][ENVIRONMENT], $data));
        $data_a_inyectar_en_consulta_local = array();
        foreach ($responseApi as $data_orden) {
            $obj = new stdClass();
            $obj->observacion = $data_orden->observacion;
            $obj->tecnico = $data_orden->tecnico;
            $obj->nameTecnico = $data_orden->nameTecnico;
            $data_a_inyectar_en_consulta_local[$data_orden->nro_msj] = $obj;
        }
        array_map(function($item) use ($data_a_inyectar_en_consulta_local) {
            if (isset($data_a_inyectar_en_consulta_local[$item->nro_msj])) {
                $item->observacion = $data_a_inyectar_en_consulta_local[$item->nro_msj]->observacion;
                $item->tecnico = $data_a_inyectar_en_consulta_local[$item->nro_msj]->tecnico;
                $item->nameTecnico = $data_a_inyectar_en_consulta_local[$item->nro_msj]->nameTecnico;
            } else {
                $item->observacion = null;
                $item->tecnico = null;
                $item->nameTecnico = null;
            }
            return $item;
        }, $result_query_local); 
        // fin bloque 
    ...
    {
        // lionel -> esta consulta se puede resolver de esta manera vvv
        // return count($this->historialNovedades($serie));
    ...
    {
        // lionel -> esta consulta se puede resolver de esta manera vvv
        // return count($this->historialDesestimados($serie));
