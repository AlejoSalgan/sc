...
class Ssti_model extends CI_Model
{
  ...
  function updateEntradaAuxiliar($entradaInfo, $id_entrada) //Modifico un remito.
  {
  ...
  function protocolos_estados_find_all($order = null)
  {
    $this->db->select('PE.*');
    $this->db->from('protocolos_estados AS PE');
    if($order) {
  ...
  function protocolos_estados_insert($data) 
  {
    $this->db->trans_start();
    $this->db->insert('protocolos_estados', $data);
    $insert_id = $this->db->insert_id();
    $this->db->trans_complete();
    return $insert_id;
  }
  
  function protocolos_estados_update($id_registro, $data) 
  {
    $this->db->where('id', $id_registro);
    return $this->db->update('protocolos_estados', $data);
  }
