...
public function get_trazabilidad_por_proyecto($proyecto, $fecha_desde, $fecha_hasta, $municipios)
{
    // Obtener los IDs de los municipios que el usuario puede ver
    $municipios_ids = array_column($municipios, 'id');
    $this->db->select('PM.id AS Protocolo, MU.descrip AS Proyecto, PM.fecha AS Fecha_del_Protocolo, PM.fecha_inicial AS Fecha_Toma_Inicial, PM.fecha_final AS Fecha_Toma_Final, PM.numero_exportacion AS Numero_Exportacion, PM.cantidad AS Cantidad_de_Archivos, PM.info_desencriptados AS Archivos_Desencriptados, PM.info_filtro_velocidad AS Filtro_Velocidad, PM.info_velocidad_0 AS Filtro_Sin_Toma_Veloc, PM.info_velocidad_150 AS Filtro_150, PM.info_danados AS Archivos_Dañados, PM.info_editables AS Registros_Editables, PM.info_aprobados AS Registros_Aprobados, PM.info_descartados AS Registros_Descartados, PM.info_verificacion AS Registros_Desc_Verificacion');
    $this->db->from('protocolos_main AS PM');
    $this->db->join('municipios AS MU', 'MU.id = PM.municipio');

    // Verificar si el proyecto es -1
    if ($proyecto == -1) {
        // Si es -1, agregar la condición IN para los municipios proporcionados
        $this->db->where_in('PM.municipio', $municipios_ids);
    } else {
        // Si no es -1, agregar la condición para un municipio específico
        $this->db->where('PM.municipio', $proyecto);
    }
    $this->db->where('PM.fecha >=', $fecha_desde);
    $this->db->where('PM.fecha <=', $fecha_hasta);
...
public function get_trazabilidad_por_proyecto_martin($proyecto, $fecha_desde, $fecha_hasta,$municipios)
{
    ...
    $this->db->select('
        PM.id AS id,
        MU.descrip AS municipio,
        PM.idequipo AS idequipo,
        PM.equipo_serie AS equipo_serie,
        PM.fecha AS fecha,
        PM.usuario AS usuario,
        PM.ip AS ip,
        PM.nro_msj AS nro_msj,
        PM.estado AS estado,
        PM.fecha_inicial AS fecha_inicial,
        PM.fecha_final AS fecha_final,
        PM.cantidad AS cantidad,
        PM.tipo_equipo AS tipo_equipo,
        PM.fecha_inicial_remoto AS fecha_inicial_remoto,
        PM.fecha_final_remoto AS fecha_final_remoto,
        PM.remoto AS remoto,
        PM.estado_remoto AS estado_remoto,
        PM.estado_exportacion AS estado_exportacion,
        PM.estado_ush AS estado_ush,
        PM.numero_exportacion AS numero_exportacion,
        PM.archivos AS archivos,
        PM.registros AS registros,
        PM.entrada AS entrada,
        PM.idexportacion AS idexportacion,
        PM.encolar AS encolar,
        PM.encolar_prioridad AS encolar_prioridad,
        PM.decripto AS decripto,
        PM.incorporacion_estado AS incorporacion_estado,
        PM.est_verificacion AS est_verificacion,
        PM.info_fecha_sincronizacion AS info_fecha_sincronizacion,
        PM.info_desencriptados AS info_desencriptados,
        PM.info_filtro_velocidad AS info_filtro_velocidad,
        PM.info_velocidad_0 AS info_velocidad_0,
        PM.info_velocidad_150 AS info_velocidad_150,
        PM.info_danados AS info_danados,
        PM.info_editables AS info_editables,
        PM.info_edicion AS info_edicion,
        PM.info_aprobados AS info_aprobados,
        PM.info_descartados AS info_descartados,
        PM.info_verificacion AS info_verificacion,
        PM.incorporacion_fecha AS incorporacion_fecha,
        PM.ts AS ts
    ');
...
/*
*Listado de material de BAJADA de PBA & RN 
*/
function getReportePBA($fecha_desde3 = NULL, $fecha_hasta3 = NULL)
{
    $sql = "                                                                              SELECT mun.descrip AS 'Proyecto',
                                                                                               obm.protocolo AS 'Protocolo',
                                                                                                    em.serie AS 'Equipo',
                                                                                              obm.fecha_alta AS 'Fecha Creación Orden',
                                                                                            obm.bajada_fecha AS 'Fecha Bajada',
                                                                                            obm.subida_fecha AS 'Fecha Ingreso de Datos',
                                                                                            obm.bajada_desde AS 'Fecha Desde',
                                                                                            obm.bajada_hasta AS 'Fecha Hasta', 
                                        IF (obm.subida_fotos IS NULL, obm.bajada_archivos, obm.subida_fotos) AS 'Cantidad de Archivos', 
    IF ((obm.bajada_archivos / eqm.divide_x) IS NULL, '0', TRUNCATE((obm.bajada_archivos / eqm.divide_x),0)) AS 'Cantidad de Registros',
                                                                                           obm.bajada_observ AS 'Observaciones Bajada',
                                                                                                   pe.estado AS 'Estado Protocolo',
                                                                                                  ep.descrip AS 'Orden Cargada por'
    FROM ordenesb_main AS obm
